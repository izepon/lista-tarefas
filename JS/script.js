...
function recuperaTarefa(tarefa){
...
function adicionarTarefa(tarefa) {
  ...
  localStorage.setItem("listaTarefas", JSON.stringify(armazenaTarefas));
}
...
function editar(idTarefa) {  
  ...
  if (li) {    
    ...
    alternarJanelaEdicao();
    // armazenaTarefas.push(listaTarefas);
...
function excluir(idTarefa) {
  ...
  if (confirmacao) {
    let indice = armazenaTarefas.findIndex((i) => i.id == idTarefa);
    if (indice < 0) {
